...
void btn_callback(uint gpio, uint32_t events) {
  if (events == 0x4) { // Quando o botão for pressionado (falling edge)
    if (gpio == BTN_PIN) {
      tempo_entrada = time_us_32();  // Registra o tempo de entrada
      flag_entrada = 1;
    }
  } else if (events == 0x8) { // Quando o botão for liberado (rising edge)
    if (gpio == BTN_PIN) {
      tempo_saida = time_us_32();  // Registra o tempo de saída
      flag_saida = 1;
    }
  }
}
